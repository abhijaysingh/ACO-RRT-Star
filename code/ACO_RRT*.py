...
class RRTStar():
    ...
    def solver(self, animation=True):
        ...
        for i in range(self.max_iter):
            ...
            self.calculate_u_exploration(near_nodes)

            c_max = np.max([self.calc_dist_to_goal(node.x, node.y) for node in self.node_list])
            if c_path is not math.inf:

                if new_node.cost + self.calc_dist_to_goal(new_node.x, new_node.y) < c_path:
                    alpha = 0.3
                    if self.node_list[last_index].cost < c_path:
                        cost = self.node_list[-1].cost
                        cost_to_go = self.calc_dist_to_goal(self.start.x, self.start.y)
                        self.calculate_u_exploitation(c_max, c_path, 0, cost, cost_to_go, 2)
                    else:
                        ...
                        self.calculate_u_exploitation(c_max, c_path, cost_to_go, 0, cost_total, 3)
                else:
                    alpha = 0
                    self.u_exploration = 0
                    self.u_exploitation = 0
            else:
                alpha = 0.1
                cost_to_goal = self.calc_dist_to_goal(new_node.x, new_node.y)
                self.calculate_u_exploitation(c_max, c_path, cost_to_goal, 0, 0, 1)
            ...
            P = (self.u_exploration, self.u_exploitation, new_node, self.end, alpha, l, path, c_path)
            s_l = self.Table.ants[l].S
            cost_to_goal = self.calc_dist_to_goal(s_l[0], s_l[1])
            cost_to_come = self.calc_distance_and_angle(self.start, new_node)[0]
            self.Table.update_ants(P, cost_to_come,cost_to_goal, c_path)
    ...
    def calculate_u_exploitation(self, c_max, c_path, cost_euclidean_x_xb, cost_graph_xb, cost_euclidean_xa_xb, state):
        """
        Calculates the utility for exploitation

        Parameters
        ----------
        c_max: float
            maximum cost to the goal
        c_path: float
            cost of the path
        cost_euclidean_x_xb: float
            euclidean distance from the current node to the new node
        cost_graph_xb: float
            cost of the graph from the new node to the end node
        cost_euclidean_xa_xb: float
            euclidean distance from the current node to the end node
        state: int  
            state of the ant

        Returns
        -------
        u_exploitation: float
    ...
    def calculate_u_exploration(self, near_nodes):
        """
        Calculates the utility for exploration
        ...
        """
        gamma = 1
        self.R = min(gamma * (np.log(len(self.node_list)) /
                     len(self.node_list))**(1 / self.d), self.eta)
    ...
    def draw_graph(self, rnd=None, path=None):
        ...
        positions = [ant.S for ant in self.Table.ants]
        plt.plot([x for (x, y) in positions], [y for (x, y) in positions], "xy") 
